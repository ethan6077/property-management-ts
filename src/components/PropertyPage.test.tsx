...
it('renders filtered propertyList when propertyFilter is set', () => {
  ...
  expect(propertyPageWrapper.find(PropertyListContainer).props().propertyList).toHaveLength(3);
  // after set the filter, only available properties will be listed
  propertyPageWrapper.setState({ propertyFilter: 'current' });
  expect(propertyPageWrapper.find(PropertyListContainer).props().propertyList).toHaveLength(2);
});
